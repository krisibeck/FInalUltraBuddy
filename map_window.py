...
class MapWindow(MDScreen):
    ...
    def popup_station_and_zoom(self, st, source, touch):
        if source.collide_point(*touch.pos):
            # print(st.name, source, touch)
            for station in self.stations:
                if station != st and station.is_open:
                    station.refresh_open_status()
            if st.dist_diff:
                display_dist = '+' + str(round(st.dist_diff, 1)) if st.dist_diff >= 0 else str(round(st.dist_diff, 1))
                st.btn.text = st.name + '\n' + f'{display_dist} km'
            self.main_map.center_on(st.lat, st.lon)
